<template>
  <div v-if="!isPlayerVisible">
    <div class="text-yellow-300 text-center font-bold mt-3 mb-8 text-3xl">
      Music App using Vue
    </div>
    <div
      v-for="(song, songIndex) in list"
      v-bind:key="song.id"
      class="flex flex-row justify-between mb-7 cursor-pointer"
    ...
    >
      ...
      <div>
        <img class="max-h-14 rounded" v-bind:src="song.src" />
      </div>
    </div>
  </div>
  <div v-if="isPlayerVisible">
